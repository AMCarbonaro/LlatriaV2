            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <Button
                  variant="primary"
                  onClick={async () => {
                    // Post to all platforms that haven't been posted yet
                    const platformsToPost = (['facebook', 'ebay', 'website'] as const).filter(
                      p => getPostingStatus(p) !== 'posted' && getPostingStatus(p) !== 'posting'
                    );
                    for (const platform of platformsToPost) {
                      handlePostToPlatform(platform);
                    }
                  }}
                  disabled={
